...
module.exports.run = async (client, msg, args) => {
  ...
  sql.get('SELECT * FROM userprofile WHERE userid = ?', [msg.author.id]).then(async (row) => {
    ...
    //Enemy discovery
    let enemiesLeft = `Enemies Defeated in Area: \`${bot.enemies_defeated}/${bot.enemies_defeated_enemies}\``;
    let question = `You find yourself in front of a ${enemy.name} holding a ${enemy.weapon_name}! Will you confront it?`;
    let stats = `Health: \`${enemy.health}\`, ${enemy.weapon_name}: \`${enemy.weapon_damage}\`, Lootchance: \`${Math.floor(enemy.lootchance)}\``;
      ...
      yes.on('collect', async (reaction, reactionMessage) => {
        ...
        utils.removeAllReactions(m, msg.guild.me.hasPermission('MANAGE_MESSAGES')).then(async () => {
          ...
          COLLECTOR_ATTACK.on('collect', async (reaction, reactionCollector) => {
            ...
            let player_damage_modified = parseFloat(player.damage - (player.damage / 2) + Math.random(player.damage)).toFixed(2);

            let player_critical_gen = Math.random();
            ...
            if (enemy.health <= 0) {
              ...
              utils.editEmbed(m, msg, `${enemy.name} defeated!`).then(async m => {
                ...
                LOOT.on('collect', async collected => {
                  ...
                  let coins = Math.floor((Math.random() * enemy.value) + 1);
